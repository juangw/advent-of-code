...
func main() {
	...
	for i := 0; i < len(campCleanupAssignments); i++ {
		...
		if campCleanupInts[0] <= campCleanupInts[2] && campCleanupInts[2] <= campCleanupInts[1] {
			totalOverlapCount++
		} else if campCleanupInts[2] <= campCleanupInts[0] && campCleanupInts[0] <= campCleanupInts[3] {
			totalOverlapCount++
		}
	}
